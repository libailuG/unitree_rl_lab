...
velocity_commands = np.array([0.2, 0.0, 0.0])
...
# ── Data saving (same structure as 4_play_a2_rough.py) ──
OBS_TERMS = [
...
def write_obs_fifo(lines, fifo_flat, prefix, history):
    """Split a flat obs (FIFO/IsaacLab) into named components and format.

    Layout (same as Isaac Lab): for each term, all dims of hist_0 come first,
    then all dims of hist_1, etc.
    E.g. joint_pos (dim=12, H=3):
      [j0_h0, j1_h0, ..., j11_h0, j0_h1, ..., j11_h1, j0_h2, ..., j11_h2]
    """
    t = fifo_flat
    assert len(t) == OBS_DIM * history, f"Expected {OBS_DIM * history}, got {len(t)}"
...
def main():
    ...
    output_dir = os.path.join(os.getcwd(), "logs", "step_data_mujoco")
                ...
                if step_count % decimation == 0:
                    ...
                    if record_phase == 2:
                        ...
                        if record_steps_collected >= 10:
                            ...
                            # Write data
                            all_lines = []
                            all_lines.append("# Auto-generated step data from 52_test_isaaclab_to_mujoco.py")
                            all_lines.append(f"# obs_dim per history step: {OBS_DIM}")
                            all_lines.append(f"# history length: {obs_history_length} → {OBS_DIM * obs_history_length} dims")
                            all_lines.append(f"# action_scale: {action_scale}, clip_actions: {clip_actions}")
                            all_lines.append("")

                            for i, fifo_flat in enumerate(rec_fifos):
                                all_lines.append("# ============================================================")
                                all_lines.append(f"#  Step {i}")
                                all_lines.append("# ============================================================")
                                all_lines.append(f"# --- policy observation ({obs_history_length} history × {OBS_DIM} = {OBS_DIM * obs_history_length}) ---")
                                write_obs_fifo(all_lines, fifo_flat, f"step{i}_obs_policy", obs_history_length)
