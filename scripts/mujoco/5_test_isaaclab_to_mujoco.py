...
def main():
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running():
            ...
            step_count += 1
            # if step_count % 100 == 0:
            #     flat_min = np.argmin(height_scanner_obs)
            #     flat_max = np.argmax(height_scanner_obs)
            #     lt_min, fw_min = np.unravel_index(flat_min, heights_2d.shape)
            #     lt_max, fw_max = np.unravel_index(flat_max, heights_2d.shape)

            #     print(f"\n--- step {step_count} ---")
            #     print(f"  obs.max = {height_scanner_obs[flat_max]:.3f}  @ idx={flat_max}  [{grid_pos_to_desc(lt_max, fw_max)}]"
