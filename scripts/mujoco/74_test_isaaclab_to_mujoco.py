...
def get_ray_caster_obs(data, sensor_name):
    ...
    valid_z.clip(min=-1.0, max=1.0, out=valid_z)
...
policy_path = os.path.join(
    os.getcwd(), "logs/rsl_rl/", "taixi_a2_velocity_rough/2026-07-30_14-50-04/",
    "exported/", "policy_260730.pt"
)
...
viewer.vopt.geomgroup[3] = 1   # 显示碰撞体 (group 3)
viewer.vopt.geomgroup[4] = 1   # 显示地面   (group 4)
# 设置初始视角
...
try:
    while viewer.is_alive:
        ...
        if step_flag:
            ...
            valid_z = get_ray_caster_obs(data, sensor_name)
            ...
            if run_first_flag:
                ...
                compute_obs(data, velocity_commands, action, valid_z, step_count * sim_dt, add_noise=False)

                data.ctrl = init_joint_pos_isaaclab[isaaclab_to_mj_act]

            else:
                if step_count % decimation == 0:
                    step_flag = False  # 完成一次策略推理后暂停
